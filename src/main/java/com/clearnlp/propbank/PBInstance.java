...
public class PBInstance implements Serializable, Comparable<PBInstance>
{
	...
	public PBInstance(String str)
	{
		...
		if (tmp.length < 7 || !PatternUtils.containsOnlyDigits(tmp[1]) || !PatternUtils.containsOnlyDigits(tmp[2]))
	...
	public boolean isNounPredicateLightVerb(CTTree tree, LanguageType lang)
	{
		if (isNounPredicate())
		{
			...
			{
				if (lang == LanguageType.ENGLISH && CTLibEn.isVerb(tree.getNode(loc)))
					return true;
			}			
		}
